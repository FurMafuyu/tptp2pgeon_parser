      ...
      if String.length line > 0 && line.[0] <> '%' then 
        ...
        if contains "file" l_low && contains "qmltp" l_low then
          ...
          if current_status <> UnknownStatus then 
            ...
            let clean_line = String.map (fun c -> if c = '\t' then ' ' else c) l_low in
            let words = String.split_on_char ' ' clean_line 
            ...
            | _ -> 
                let rec find_constant_before_version = function
                  | status_candidate :: version_str :: _ when String.length version_str > 0 && version_str.[0] = 'v' ->
                      string_to_status status_candidate
                  | _ :: rest -> find_constant_before_version rest
                  | [] -> current_status
                in
                let status = find_constant_before_version words in
                parse_lines is_qmltp status
